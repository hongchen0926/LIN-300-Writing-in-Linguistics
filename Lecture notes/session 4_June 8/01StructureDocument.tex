...
%\documentclass[12pt, letterpaper]{article} % specify the font and paper settings
%\usepackage[top=1in, bottom=1in, left=1in, right=1in]{geometry} % specify the margin

\usepackage{times}%font specification
...
\author{Hongchen Wu\\
		\texttt{hongchen.wu@stonybrook.edu}}  %\texttt formats the text to a typewriter style font
\date{\today}  %\today is replaced with the current date
\maketitle

\begin{abstract}
In this article, I shall discuss some of the fundamental topics in
producing a structured document with \LaTeX.  This document itself does not go into
much depth, but is instead the output of an example of how to implement
structure. If you want to learn more about \LaTeX{} after class, you can Google ``latex for linguistics'', or schedule a meeting with me (I'll try my best to tell you more about \LaTeX{} or help you fix some bug).  \\ %% please pay attention to how LaTex produce double quote makers. 
\\ % make an empty line
\emph{Keywords}: tutorial session, structure, \LaTeX{} % keywords of a paper
\end{abstract}
